...
public class JWebSocketSubSystemSample {
	...
	public JWebSocketSubSystemSample() {
		...
		LoggingConfig lLoggingConfig = new LoggingConfig("console", // target
		...
		EngineConfiguration lEngineConfig = new EngineConfig("tcp0", // id
				"org.jwebsocket.tcp.engines.TCPEngine", // name
				"-", // jar, needs to be in classpath, i.e. embedded in
				// .jar'/manifest
				8787, // port
				120000, // default session timeout
				JWebSocketCommonConstants.DEFAULT_MAX_FRAME_SIZE, // max
				// framesize
		...
		if (mEngine != null) {
			...
			ServerConfiguration lServerConfig = new ServerConfig("ts0", // id
