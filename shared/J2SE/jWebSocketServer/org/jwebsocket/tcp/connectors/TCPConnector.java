...
public class TCPConnector extends BaseConnector {
	...
	public TCPConnector(WebSocketEngine aEngine, Socket aClientSocket) {
		...
		try {
			...
			mLog.error(ex.getClass().getSimpleName() + " instantiating " + getClass().getSimpleName() + ": " + ex.getMessage());
		}
	}
	...
	public void stopConnector(CloseReason aCloseReason) {
		...
		try {
			...
			if (mLog.isDebugEnabled()) {
				mLog.info(ex.getClass().getSimpleName() + " while stopping TCP connector (" + aCloseReason.name() + ") on port " + lPort + ": " + ex.getMessage());
			}
		}
	}
							...
							if (lStart >= 0) {
								if (pos <= lMaxFrameSize) {
									...
									try {
										...
										mLog.error(ex.getClass().getSimpleName() + " in processPacket of connector " + connector.getClass().getSimpleName() + ": " + ex.getMessage());
									}
								} else {
