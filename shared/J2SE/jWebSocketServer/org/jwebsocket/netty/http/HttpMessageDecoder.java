 ...
 * <td>The maximum length of the initial line (e.g. {@code "GET / HTTP/1.0"} or
 * {@code "HTTP/1.0 200 OK"}) If the length of the initial line exceeds this
 ...
 * (or the length of each chunk) exceeds this value, the content or chunk will
 * be split into multiple {@link HttpChunk}s whose length is {@code
 * maxChunkSize} at maximum.</td>
 * </tr>
 * </table>
 * 
 * <h3>Chunked Content</h3>
 * 
 * If the content of an HTTP message is greater than {@code maxChunkSize} or the
 * transfer encoding of the HTTP message is 'chunked', this decoder generates
 * one {@link HttpMessage} instance and its following {@link HttpChunk}s per
 * single HTTP message to avoid excessive memory consumption. For example, the
 ...
 * <li>The first {@link HttpChunk} whose content is {@code 
 * 'abcdefghijklmnopqrstuvwxyz'},</li>
 * <li>The second {@link HttpChunk} whose content is {@code '1234567890abcdef'},
 * and</li>
 ...
 * {@link ChannelPipeline}. However, please note that your server might not be
 * as memory efficient as without the aggregator.
 * 
 * <h3>Extensibility</h3>
 * 
 * Please note that this decoder is designed to be extended to implement a
 * protocol derived from HTTP, such as <a
 * href="http://en.wikipedia.org/wiki/Real_Time_Streaming_Protocol">RTSP</a> and
 * <a href="http://en.wikipedia.org/wiki/Internet_Content_Adaptation_Protocol">
 * ICAP</a>. To implement the decoder of such a derived protocol, extend this
 * class and implement all abstract methods properly.
 * 
 * @author <a href="http://www.jboss.org/netty/">The Netty Project</a>
 * @author Andy Taylor (andy.taylor@jboss.org)
 * @author <a href="http://gleamynode.net/">Trustin Lee</a>
 * @version $Rev: 2287 $, $Date: 2011/06/17 08:43:50 $
 * 
 * @apiviz.landmark
 */
public abstract class HttpMessageDecoder extends ReplayingDecoder<HttpMessageDecoder.State> {
	 ...
	 * @author <a href="http://www.jboss.org/netty/">The Netty Project</a>
	 * @author <a href="http://gleamynode.net/">Trustin Lee</a>
	 * @version $Rev: 2287 $, $Date: 2010-06-23 05:35:00 +0200 (Wed, 23 Jun
	 *          2010) $
	 * 
	 * @apiviz.exclude
	 */
	...
	/**
	 * Creates a new instance with the default {@code maxInitialLineLength
	 * (4096} , {@code maxHeaderSize (8192)}, and {@code maxChunkSize (8192)}.
	 */
	...
	protected Object decode(ChannelHandlerContext ctx, Channel channel, ChannelBuffer buffer, State state) throws Exception {
		...
		case READ_HEADER: {
			...
			// We return null here, this forces decode to be called again where
			// we will decode the content
			return null;
		}
		case READ_VARIABLE_LENGTH_CONTENT: {
			if (content == null) {
			...
			// this will cause a replay error until the channel is closed where
			// this will read what's left in the buffer
			content.writeBytes(buffer.readBytes(buffer.readableBytes()));
			return reset();
		}
		case READ_VARIABLE_LENGTH_CONTENT_AS_CHUNKS: {
			// Keep reading data as a chunk until the end of connection is
			// reached.
		...
		case READ_FIXED_LENGTH_CONTENT_AS_CHUNKS: {
			 ...
			 * content. basically, read chunk size, read chunk, read and ignore
			 * the CRLF and repeat until 0
			 */
