	...
	public List getResultColumns(ResultSet aResultSet, int aColCount) {
		...
		try {
			for (int i = 1; i <= aColCount; i++) {
				...
				lDataRow.add(lObj);
				/*
				 * if (obj == null) { // nothing todo } else if (obj instanceof
				 * byte[]) { blobStr = new String((byte[]) obj);
				 * lDataRow.append(Tools.stringToJSON(blobStr));
				 * 
				 * } else if (obj instanceof java.sql.Date) { lDataRow.append(
				 ...
				 * 
				 * } else { lDataRow.append(obj.toString()); }
				 * 
				 * if (i < aColCount) {
