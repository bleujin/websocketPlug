 ...
 * <tt>BaseStream</tt> class. The <tt>BaseStream</tt> implements an internal
 * queue to which messages can be posted. The message then are broadcasted to
 * the registered clients. Therefore the <tt>BaseStream</tt> class maintains a
 * list of clients. A certain client can register at or unregister from the
 * stream. Basically streams send their messages only to clients that are
 * registered at a stream.
 * 
 * @author aschulze
 */
