...
import net.ion.websocket.common.filter.TokenFilterChain;
...
import net.ion.websocket.common.listener.WebSocketServerTokenEvent;
import net.ion.websocket.common.listener.WebSocketServerTokenListener;
import net.ion.websocket.common.plugin.TokenPlugInChain;
import net.ion.websocket.common.server.BaseServer;
import net.ion.websocket.common.server.FragmentedTokenBuilder;
...
public class TokenServer extends BaseServer {
	...
	private static final String NS_TOKENSERVER = ServerConstants.NS_BASE + ".tokenserver";
	...
	public TokenServer(ServerConfiguration aServerConfig) {
		super(aServerConfig);
		plugChain = new TokenPlugInChain(this);
...
//		mBlockingQueueSize = aServerConfig.getThreadPoolConfig().getBlockingQueueSize();
		...
		mCorePoolSize = 10;
		mMaximumPoolSize = 100;
		mKeepAliveTime = 60;
		mBlockingQueueSize = 1000;
	...
	public void startServer() throws WebSocketException {
		...
		mCachedThreadPool = new ThreadPoolExecutor(mCorePoolSize, mMaximumPoolSize, mKeepAliveTime, TimeUnit.SECONDS, new ArrayBlockingQueue<Runnable>(mBlockingQueueSize));
		mIsAlive = true;
	...
	public void removePlugIn(WebSocketPlugIn aPlugIn) {
		plugChain.removePlugIn(aPlugIn);
	}
	...
	public void engineStarted(WebSocketEngine aEngine) {
		...
		plugChain.engineStarted(aEngine);
	}
	...
	public void engineStopped(WebSocketEngine aEngine) {
		...
		plugChain.engineStopped(aEngine);
	}
	...
	public void connectorStarted(WebSocketConnector aConnector) {
		...
		if ((lFormat != null) && (lFormat.equals(CommonConstants.WS_FORMAT_JSON) || lFormat.equals(CommonConstants.WS_FORMAT_XML) || lFormat.equals(CommonConstants.WS_FORMAT_CSV))) {
			...
			plugChain.connectorStarted(aConnector);
		}
	...
	public void connectorStopped(WebSocketConnector aConnector, CloseReason aCloseReason) {
		...
		if (aConnector.getBool(VAR_IS_TOKENSERVER)) {
			...
			plugChain.connectorStopped(aConnector, aCloseReason);
		}
		...
		if (aConnector.getBool(VAR_IS_TOKENSERVER)) {
			...
			if (lToken != null) {
				...
				if (lRunReqInOwnThread) {
					...
					if ("org.jwebsocket.plugins.system".equals(lToken.getNS()) && "fragment".equals(lToken.getType())) {
	...
	public void sendToken(String aEngineId, String aConnectorId, Token aToken) {
		...
		WebSocketConnector lTargetConnector = getConnector(aConnectorId);
	...
	public void broadcastFiltered(Token aToken, String aFilterID, String aFilterName) {
		if (mLog.isDebugEnabled()) {
			mLog.debug("Broadcasting token '" + aToken + "' to all token based " + "connectors that belong to the filter '" + aFilterID + "' called '" + aFilterName + "'...");
		}
	...
	/**
	 * iterates through all connectors of all engines and sends the token to each connector. The token format is considered for each connection individually so that the application can broadcast a token to all kinds of clients.
	 * 
	 * @param aSource
	 * @param aToken
	...
	public Token createNotAuthToken(Token aInToken) {
		...
		/*
		 * Token lResToken = createResponse(aInToken); lResToken.setInteger("code", -1); lResToken.setString("msg", "not authenticated");
		 */
		return lResToken;
	}
	...
	public Token createAccessDenied(Token aInToken) {
		...
		/*
		 * Token lResToken = createResponse(aInToken); lResToken.setInteger("code", -1); lResToken.setString("msg", "access denied");
		 */
		return lResToken;
	}
	 ...
	 * @param aMessage
	 */
	public void sendErrorToken(WebSocketConnector aConnector, Token aInToken, int aErrCode, String aMessage) {
	...
	public TokenPlugInChain getPlugInChain() {
		return (TokenPlugInChain) plugChain;
	}
