...
import net.ion.websocket.common.config.JWebSocketCommonConstants;
import net.ion.websocket.common.config.JWebSocketServerConstants;
...
import net.ion.websocket.common.packet.processor.CSVProcessor;
import net.ion.websocket.common.packet.processor.JSONProcessor;
import net.ion.websocket.common.packet.processor.XMLProcessor;
...
public class TokenServer extends BaseServer {
	...
	public void connectorStarted(WebSocketConnector aConnector) {
		...
		if ((lSubProt != null) && (lSubProt.equals(JWebSocketCommonConstants.SUB_PROT_JSON) || lSubProt.equals(JWebSocketCommonConstants.SUB_PROT_CSV) || lSubProt.equals(JWebSocketCommonConstants.SUB_PROT_XML))) {
	...
	public void processPacket(WebSocketEngine aEngine, final WebSocketConnector aConnector, WebSocketPacket aDataPacket) {
		...
		if (aConnector.getBool(VAR_IS_TOKENSERVER)) {
			...
			if (lToken != null) {
				...
				/*
				 * // before forwarding the token to the plug-ins push it
				 * through filter chain FilterResponse filterResponse =
				 * getFilterChain().processTokenIn(aConnector, lToken);
				 * 
				 * // only forward the token to the plug-in chain // if filter
				 * chain does not response "aborted" if
				 * (!filterResponse.isRejected()) {
				 * getPlugInChain().processToken(aConnector, lToken); // forward
				 * the token to the listener chain List<WebSocketServerListener>
				 * lListeners = getListeners(); WebSocketServerTokenEvent lEvent
				 * = new WebSocketServerTokenEvent(aConnector, this); for
				 * (WebSocketServerListener lListener : lListeners) { if
				 * (lListener != null && lListener instanceof
				 * WebSocketServerTokenListener) {
				 * ((WebSocketServerTokenListener)
				 * lListener).processToken(lEvent, lToken); } } }
				 */
			} else {
