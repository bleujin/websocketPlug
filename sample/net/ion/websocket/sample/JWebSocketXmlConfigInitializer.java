...
public final class JWebSocketXmlConfigInitializer implements WebSocketInitializer {
	...
	public void initializeLogging() {
		...
		Logging.initLogs(lLoggingConfig.getLevel(), lLoggingConfig.getAppender(), lLoggingConfig.getFilename(), lLoggingConfig.getPattern(), lLoggingConfig.getBufferSize(), new String[] { "%JWEBSOCKET_HOME%/logs", "%CATALINA_HOME%/logs" });
		mLog = Logging.getLogger(JWebSocketXmlConfigInitializer.class);
		if (mLog.isDebugEnabled()) {
			mLog.debug("Logging settings" + ": appender: " + lLoggingConfig.getAppender() + ", filename: " + lLoggingConfig.getFilename() + ", level: " + lLoggingConfig.getLevel() + ", buffersize: " + lLoggingConfig.getBufferSize() + ", pattern: " + lLoggingConfig.getPattern());
		}
		if (mLog.isDebugEnabled()) {
		...
		for (ServerConfig lServerConfig : lServerConfigs) {
			...
			try {
				...
				if (lServerClass == null) {
					...
					// jarFilePath may be null if .jar is included in server
					// bundle
				...
				mLog.error("Couldn't load the jar file for server, make sure jar file '" + lJarFilePath + "' exists and name is correct.", e);
			} catch (ClassNotFoundException e) {
				mLog.error("Server class '" + lServerConfig.getName() + "'@'" + lJarFilePath + "' not found.", e);
			} catch (InstantiationException e) {
				mLog.error("Server class '" + lServerConfig.getName() + "' could not be instantiated.", e);
			} catch (IllegalAccessException e) {
				mLog.error("Illegal Access Exception while intializing server '" + lServerConfig.getName() + "'.", e);
			} catch (NoSuchMethodException e) {
				mLog.error("No constructor found with given 1 arguments", e);
			} catch (InvocationTargetException e) {
				mLog.error("Exception invoking server object.", e);
			}
		}
		return lServers;
	}
	...
	public Map<String, List<WebSocketPlugIn>> initializePlugins() {
		...
		for (PluginConfig lPluginConfig : mConfig.getPlugins()) {
			try {
				...
				if (lPluginClass != null) {
					/*
					 * Constructor<WebSocketPlugIn> ctor =
					 * pluginClass.getDeclaredConstructor();
					 * ctor.setAccessible(true); Object lObj =
					 * ctor.newInstance(new Object[]{});
					 * log.debug("lObj.classname = " +
					 * lObj.getClass().getName()); Object plugin = null; try {
					 * plugin = lObj; log.info(
					 * "lObj instanceof WebSocketPlugIn " + ( lObj instanceof
					 * WebSocketPlugIn ? "YES" : "NO" ) ); } catch (Exception
					 * ex) { log.error( ex.getClass().getSimpleName() +
					 * " while instantiating class '" + pluginConfig.getName() +
					 * "'."); }
					 */
				...
				mLog.error("Couldn't load the jar file for plugin, make sure the jar file exists and the name is correct.", ex);
			} catch (ClassNotFoundException ex) {
				mLog.error("Plugin class '" + lPluginConfig.getName() + "' not found.", ex);
			} catch (InstantiationException ex) {
				mLog.error("Plugin class '" + lPluginConfig.getName() + "' could not be instantiated.", ex);
			} catch (IllegalAccessException ex) {
				mLog.error("Illegal Access Exception while instantiating plugin.", ex);
			} catch (Exception ex) {
				mLog.error(ex.getClass().getSimpleName() + " while instantiating plugin '" + lPluginConfig.getName() + "'.", ex);
			}
		}
		return lPluginMap;
	}
		...
		for (FilterConfig lFilterConfig : mConfig.getFilters()) {
			try {
				...
				if (lFilterClass != null) {
					...
					// TODO: Also set settings, id and name once these are
					// available
				...
				mLog.error("Couldn't Load the jar file for filter, make sure jar file exists and name is correct.", e);
			} catch (ClassNotFoundException e) {
				mLog.error("Filter class not found.", e);
			} catch (InstantiationException e) {
				mLog.error("Filter class could not be instantiated.", e);
			} catch (IllegalAccessException e) {
				mLog.error("Illegal Access Exception while intializing filter.", e);
			} catch (NoSuchMethodException e) {
				mLog.error("No Constructor found with given 3 arguments.", e);
			} catch (InvocationTargetException e) {
				mLog.error("Exception invoking filter object.", e);
			}
		}
		return lFilterMap;
	}
