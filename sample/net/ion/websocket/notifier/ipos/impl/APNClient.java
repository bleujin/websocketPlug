 ...
 */
package net.ion.websocket.notifier.ipos.impl;
...
import net.ion.websocket.notifier.ipos.APNClientService;
import net.ion.websocket.notifier.ipos.UnreachableDevicesCallback;
import net.ion.websocket.notifier.ipos.data.APNServerConfig;
import net.ion.websocket.notifier.ipos.data.NotificationDevice;
import net.ion.websocket.notifier.ipos.data.NotificationPayLoad;
import net.ion.websocket.notifier.ipos.network.Network;
import net.ion.websocket.notifier.ipos.network.NetworkConnectionStateListener;
import net.ion.websocket.notifier.ipos.network.NetworkDataListener;
import net.ion.websocket.notifier.ipos.util.PKIUtils;
 ...
 * <sub>An APNs client for queueing and dispatching notifications to the
 * APNs.</sub></h2>
 * 
 * <p>
 * TODO
...
public class APNClient implements APNClientService, NetworkConnectionStateListener, NetworkDataListener {
	...
	/**
	 * Create a new {@link APNClient} instance by setting up the PKIX identity
	 * and trust to reasonable defaults from the given parameters.
	 * 
	 * @param keyStore
	 *            The keystore which provides all required SSL keys and
	 *            certificates.
	 * @param privateKeyPassword
	 *            The password which protects the required <code>keyStore</code>
	 *            's private key.
	 * @param serverConfig
	 *            The {@link APNServerConfig} that determines the host
	 *            configuration of the Apple Push Notification server and
	 *            Feedback service.
	 * 
	 * @throws UnrecoverableKeyException
	 *             The private key could not be accessed from the
	 *             <code>keyStore</code>. Perhaps the provided
	 *             <code>privateKeyPassword</code> is incorrect.
	 * @throws NoSuchAlgorithmException
	 *             The <code>keyStore</code> provider does not support the
	 *             necessary algorithms.
	 * @throws KeyStoreException
	 *             The <code>keyStore</code> had not been properly
	 *             loaded/initialized or is corrupt.
	 */
	public APNClient(final KeyStore keyStore, final String privateKeyPassword, final APNServerConfig serverConfig) throws UnrecoverableKeyException, NoSuchAlgorithmException, KeyStoreException {

		this(PKIUtils.createKeyManagerFactory(keyStore, privateKeyPassword), PKIUtils.createTrustManagerFactory(keyStore), serverConfig);
	}

	/**
	 * Create a new {@link APNClient} instance using a custom configured
	 * {@link KeyManagerFactory} and {@link TrustManagerFactory} to provide PKIX
	 * identity and trust.
	 * 
	 * @param trustManagerFactory
	 *            The factory that will create the SSL context's
	 *            {@link TrustManager}s.
	 ...
	 * 
	 * @param serverAddress
	 *            The hostname and port of the remote server socket to create an
	 *            SSL engine for.
	 * @param protocol
	 *            The SSL/TLS protocol to use for secure transport encryption.
	 *            <p>
	 *            Valid values depend on what is supported by the
	 *            <code>sslProvider</code>, but generally speaking there is:
	 *            <code>SSL, SSLv2, SSLv3, TLS, TLSv1,
	 *                            TLSv1.1, SSLv2Hello</code>.
	 *            </p>
	 * @param trustManagerFactory
	 *            The factory that will create the SSL context's
	 *            {@link TrustManager}s.
	 ...
	 *             The SSL context could not be initialized using the available
	 *             private keys.
	 * @see <a
	 *      href="http://java.sun.com/javase/6/docs/technotes/guides/security/StandardNames.html#jssenames">JSSE
	 *      Protocol Names</a>
	 */
	private static SSLEngine createSSLEngine(final InetSocketAddress serverAddress, final String protocol, final KeyManagerFactory keyManagerFactory, final TrustManagerFactory trustManagerFactory) throws NoSuchAlgorithmException, KeyManagementException {
	...
	public boolean queueNotification(final NotificationDevice device, final JSONObject customData, final JSONObject... notificationPayLoads) {
		...
		// All done with adding stuff to the notification data buffer. Flip &
		// queue it.
	...
	/**
	 * Calculate the byte size of the interface for sending a certain
	 * notification.
	 * 
	 * @param tokenLength
	 *            The byte size of the {@link NotificationDevice}'s token that
	 *            identifies the notification's destination.
	 * @param payLoadLength
	 *            The byte size of the {@link NotificationPayLoad} when
	 *            serialized and encoded with the character set as defined by
	 *            {@link #getPayloadEncoding()}.
	 * 
	 * @return The byte size of the notification interface that will be sent to
	 *         the APNs.
	 */
	 ...
	 * 
	 * <p>
	 * This operation will establish a connection to the configured APNs if one
	 * is not active already. Note that the connection will not be terminated
	 * automatically. You are responsible for calling {@link #closeAPNs()} when
	 * you determine you won't be dispatching any more messages soon.
	 * </p>
	 * 
	 * @param notificationInterface
	 *            The {@link ByteBuffer} containing the raw interface of the
	 ...
	 *             private keys.
	 */
	public void dispatch(final ByteBuffer notificationInterface) throws IOException, KeyManagementException, NoSuchAlgorithmException {
	 ...
	 * {@inheritDoc}
	 */
	public void fetchUnreachableDevices(final UnreachableDevicesCallback callback) throws IOException, KeyManagementException, NoSuchAlgorithmException {
	 ...
	 * 
	 * <p>
	 * Uses private key entries from the <code>keyStore</code> it was created
	 * with to provide client authentication with the server.
	 * </p>
	 * 
	 * @return The factory for {@link KeyManager}s which provide the client
	 *         identity.
	 */
	...
	/**
	 * Change the {@link KeyManagerFactory} that is used for client
	 * identification during SSL negotiation.
	 * 
	 * @param keyManagerFactory
	 *            The factory for {@link KeyManager}s which provide the client
	 *            identity.
	 */
	 ...
	 * 
	 * <p>
	 * The trust factory provides simple trust for each trusted certificate in
	 * the <code>keyStore</code> it was created with.
	 * </p>
	 * 
	...
	/**
	 * Change the {@link TrustManagerFactory} that is used to provide trust of
	 * server identification during SSL negotiation.
	 * 
	 * @param trustManagerFactory
	...
	/**
	 * @return The server configuration that is used to establish communication
	 *         to the Apple services.
	 */
	...
	/**
	 * @param serverConfig
	 *            The server configuration that is used to establish
	 *            communication to the Apple services.
	 */
	...
	/**
	 * The Apple specifications (at the time of this writing) define the byte
	 * order to be {@link ByteOrder#BIG_ENDIAN}.
	 * 
	 * @return The {@link ByteOrder} of bytes sent to the Apple Push
	 *         Notification server's raw interface.
	 */
	...
	/**
	 * <p>
	 * <b>Do not modify this property unless you have a very good reason to do
	 * so.</b>
	 * </p>
	 * 
	 * @param byteOrder
	 ...
	 *            The maximum allowed byte size of the serialized
	 *            {@link NotificationPayLoad} encoded with
	 *            {@link #getPayloadEncoding()}.
	 */
	...
	/**
	 * The Apple specifications (at the time of this writing) define the maximum
	 * payload byte size to be <code>256 bytes</code>.
	 * 
	 * @return The maximum allowed byte size of the serialized
	 *         {@link NotificationPayLoad} encoded with
	 *         {@link #getPayloadEncoding()}.
	 */
	...
	/**
	 * Obtain a reference to the {@link Network} framework used by this
	 * {@link APNClient} for its network connectivity.
	 * 
	 * <p>
	 * <b>Accessing the {@link Network} framework directly is strongly
	 * discouraged.</b> {@link APNClient} provides an interface for all features
	 * you should need. This method is mostly here for advanced usage. Going
	 * behind the back of {@link APNClient} is ill-advised. You have been
	 * warned.
	 * </p>
	 * 
	 ...
	 * {@link Network#registerConnectionStateListener(NetworkConnectionStateListener)}
	 * . However, in the interest of encapsulation, only notifications about the
	 * connection to the APNs will be relayed.
	 * </p>
	 * 
	 * @param listener
	...
	/**
	 * Unregister an object from receiving APNs network connection state
	 * updates. The object will no longer receive state updates.
	 * 
	 * @param listener
	 *            The object that used to be interested in network state changes
	 *            but no longer is.
	 */
	...
	public void connected(final SocketChannel channel) {
		...
		// Forward this event to our own state listeners if it's about the APNs
		// connection.
		if (channel == apnsChannel)
	...
	public void received(final ByteBuffer dataBuffer, final SocketChannel channel) {
		...
		if (channel == feedbackChannel) {
			...
			// Transfer the data into the feedback buffer and make it ready for
			// reading.
			...
			// Parse the bytes in the feedbackBuffer in as uninstalled device
			// records.
			while (feedbackBuffer.remaining() > 0)
				try {
					feedbackBuffer.mark();
					...
					// Not enough bytes in the dataBuffer for a whole record;
					// undo our last read operations.
	 ...
	 * 
	 * <p>
	 * <b>Note:</b> This does not close any possible connections to the Apple
	 * Push Notification Feedback service. This connection is expected to close
	 * automatically when Apple finished dumping its queue. Use
	 * {@link #closeFeedbackService()} if for some reason you want to do this
	 * anyway.
	 * </p>
	 */
	...
	/**
	 * Close any active connection to the Apple Push Notification Feedback
	 * service.
	 */
