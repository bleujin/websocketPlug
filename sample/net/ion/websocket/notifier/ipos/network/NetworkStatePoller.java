 ...
 * @author lhunath
 */
public class NetworkStatePoller extends Poller<NetworkStatePoller.State, SocketChannel> implements NetworkServerStateListener, NetworkConnectionStateListener {

	/**
	 * Create a new {@link NetworkStatePoller} instance. Register it on the
	...
	/**
	 * Create a new {@link NetworkStatePoller} instance that listens on the
	 * given network.
	 * 
	 * @param network
	...
	public enum State {
		...
		/**
		 * A new connection has been accepted by a listening socket. The element
		 * is the new connection's socket.
		 */
		ACCEPTED,

		/**
		 * A connection has been established to a remote server. The element is
		 * the connection's socket.
		 */
		CONNECTED,

		/**
		 * An active connection has been terminated. The element is the
		 * connection's socket.
		 */
		CLOSED
	}
