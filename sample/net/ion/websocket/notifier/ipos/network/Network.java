 ...
 */
package net.ion.websocket.notifier.ipos.network;
...
/**
 * <h2>{@link Network}<br>
 * <sub>A non-blocking single-threaded TCP network layer with SSL/TLS
 * support.</sub></h2>
 * <p/>
 * <p>
 * TODO
...
public class Network implements Runnable {
	...
	// Collections that are modified by calling threads and the networking
	// thread.
	...
	private Selector selector; // TODO: Synchronize all access to/of the
								// selector.
	...
	/**
	 * Start this network by executing a networking thread. When a networking
	 * thread begins, it invokes {@link #bringUp()}.
	 */
	...
	/**
	 * Stop this network's thread. This will cause {@link #bringDown()} if the
	 * network is still up.
	 */
	...
	/**
	 * @return <code>true</code> if the network thread has been started and is
	 *         currently running.
	 */
	 ...
	 * <p/>
	 * This initializes the networking {@link Selector} making it possible to
	 * start network operations. It does not start the networking thread. See
	 * {@link #startThread()} to start the network and execute a networking
	 * thread for it. This method is mostly helpful for manually bringing the
	 * network in an existing thread down and up again.
	 */
	 ...
	 * <p/>
	 * This closes the networking {@link Selector} causing all connections to be
	 * terminated.
	 */
	...
	/**
	 * @return <code>true</code>: The network is ready for network event
	 *         processing (binding, accepting, connecting, reading and writing).
	 */
	...
	/**
	 * Bind a socket on the wildcard address at a specified port and listen for
	 * connections.
	 * 
	 * @param port
	 *            The local port to bind on.
	 * @param sslEngine
	 *            If you want to use SSL/TLS to encrypt the data sent over
	 *            connections established from this socket, specify an SSL
	 *            engine that has been initialized to for communication with
	 *            remote clients. If you just want plain-text communication,
	 *            pass <code>null</code> here.
	 * 
	 * @return The channel that will be listening for connections.
	 * 
	 * @throws IOException
	 *             If the socket is already bound, the bind address is
	 *             unavailable or the operation failed or was denied for some
	 *             other reason.
	 */
	public ServerSocketChannel bind(final int port, final SSLEngine sslEngine) throws IOException {

		return bind(new InetSocketAddress(port), sslEngine);
	}

	/**
	 * Bind a socket on the interface defined by the given address at a
	 * specified port and listen for connections.
	 * 
	 * @param address
	 ...
	 *             other reason.
	 */
	public ServerSocketChannel bind(final InetAddress address, final int port, final SSLEngine sslEngine) throws IOException {

		return bind(new InetSocketAddress(address, port), sslEngine);
	}

	/**
	 * Bind a socket on the interface defined by the given hostname at a
	 * specified port and listen for connections.
	 * 
	 * @param hostname
	 *            The hostname that resolves to the interface address of the
	 ...
	 *             other reason.
	 */
	public ServerSocketChannel bind(final String hostname, final int port, final SSLEngine sslEngine) throws IOException {
	 ...
	 * 
	 * @param socketAddress
	 *            The socket address that defines the interface and port to bind
	 *            the socket on.
	 * @param sslEngine
	 *            If you want to use SSL/TLS to encrypt the data sent over
	 *            connections established from this socket, specify an SSL
	 ...
	 *             other reason.
	 */
	public ServerSocketChannel bind(final SocketAddress socketAddress, final SSLEngine sslEngine) throws IOException {
	 ...
	 * 
	 * @throws IOException
	 *             If the channel is in an unexpected state or the connection
	 *             couldn't be accepted for some other reason.
	 */
	private void accept(final ServerSocketChannel serverChannel) throws IOException {
		...
		// Register the SSL engine for this connection socket, if SSL/TLS is
		// enabled on the server socket.
		SSLEngine sslEngine = sslEngines.get(serverChannel);
		if (sslEngine != null)
			sslEngines.put(connectionChannel, sslEngine);

		// New connection; configure it for non-blocking and read what it has to
		// say.
	 ...
	 * 
	 * @param hostname
	 *            The name that resolves to the address that defines the
	 *            destination host to connect to.
	 * @param port
	 *            The port of the remote listening socket on the given host to
	 *            connect to.
	 * @param sslEngine
	 *            If you want to use SSL/TLS to encrypt the data sent over this
	 *            connection, specify an SSL engine that has been initialized to
	 *            for communication with the remote server. If you just want
	 *            plain-text communication, pass <code>null</code> here.
	 * 
	 * @return The channel on which the connection runs.
	 * 
	 * @throws IOException
	 *             If a channel couldn't be assigned and configured or a
	 *             connection couldn't be initiated (or completed in blocking
	 *             mode).
	 */
	public SocketChannel connect(final String hostname, final int port, final SSLEngine sslEngine) throws IOException {
	 ...
	 *             mode).
	 */
	public SocketChannel connect(final InetAddress hostAddress, final int port, final SSLEngine sslEngine) throws IOException {
	 ...
	 * 
	 * @param socketAddress
	 *            The address that defines the destination host and port to
	 *            connect to.
	 * @param sslEngine
	 *            If you want to use SSL/TLS to encrypt the data sent over this
	 *            connection, specify an SSL engine that has been initialized to
	 ...
	 *             mode).
	 */
	public SocketChannel connect(final InetSocketAddress socketAddress, final SSLEngine sslEngine) throws IOException {
	...
	/**
	 * Finish a connection initiated by
	 * {@link #connect(InetSocketAddress, SSLEngine)}.
	 * 
	 * @param socketChannel
	 *            The channel on which the connection happening.
	 * 
	 * @throws IOException
	 *             If the connection couldn't be established. (Channel in an
	 *             unexpected state or connection timeout, ..)
	 */
	private void finishConnect(final SocketChannel socketChannel) throws IOException {
	...
	/**
	 * Read available chatter from the given socket and
	 * {@link #notifyRead(ByteBuffer, SocketChannel)} it.
	 * 
	 * @param socketChannel
	 *            The socket to read input from.
	 * 
	 * @throws IOException
	 *             If the channel socket couldn't be read from.
	 */
	private void read(final SocketChannel socketChannel) throws IOException {
		...
		// Read available connection bytes until either the read buffer is full
		// or all available bytes have been read.
		int bytesRead = socketChannel.read(readBuffer);

		// See if read buffer filled up completely; if so, make it bigger for
	 ...
	 * <p/>
	 * <p>
	 * <b>Use the return value for processing, not the original data buffer!</b>
	 * <br>
	 * The buffer might have been reallocated in which case the original buffer
	 * is obsolete.
	 * </p>
	 * 
	 * @param readBuffer
	 *            The buffer that contains the network data, ready to be read
	 *            (position set to zero, limit set to the end of the received
	 *            network data).
	 * @param socketChannel
	 *            The channel over which the network data was received.
	 * @param dataBuffer
	 *            The buffer of application data, ready to be written/appended
	 *            to. After this operation it will be ready to be read (position
	 *            set to zero, limit set to the end of the application data).
	 * 
	 * @return The (possibly new) dataBuffer.
	 * 
	 * @throws IOException
	 *             If the read buffer couldn't be unwrapped by the SSL Engine.
	 */
	private ByteBuffer toApplicationData(final ByteBuffer readBuffer, final SocketChannel socketChannel, final ByteBuffer dataBuffer) throws IOException {

		ByteBuffer newDataBuffer = dataBuffer;

		SSLEngine sslEngine = sslEngines.get(socketChannel);
		if (sslEngine != null) {
			// SSL/TLS: Decrypt network data into application data (or into
			// nothingness!).
			...
			while (true) {
				...
				case BUFFER_UNDERFLOW:
					// Not enough network data collected for a whole SSL/TLS
					// packet.
				...
				case OK:
					logger.dbg("[>>>>: %s] SSL %s - %s: Produced %d bytes application data", //
							nameChannel(socketChannel), sslEngineResult.getStatus(), sslEngineResult.getHandshakeStatus(), sslEngineResult.bytesProduced());
					break;
				}
			...
			// Make the application data available and add new network data
			// after what's left unprocessed.
			newDataBuffer.flip();
			readBuffer.compact();
		...
		// Plain Text: Copy network data to application data and prepare both
		// buffers for their next operations.
		if (newDataBuffer.remaining() < readBuffer.remaining()) {
			// Not enough space in the dataBuffer for the readBuffer's data;
			// make it bigger.
	 ...
	 * 
	 * @throws IOException
	 *             If the channel socket couldn't be written to.
	 */
	private void write(final SocketChannel socketChannel) throws IOException {

		// Lock this connection's write queue.
		synchronized (writeQueueLocks.get(socketChannel)) {
			if (!socketChannel.isOpen() || !socketChannel.isConnected() || socketChannel.isConnectionPending() || socketChannel.socket().isOutputShutdown())
				return;

			// Obtain the application data write queue buffer. If none is
			// allocated yet, make a dummy empty one.
			...
			if (writeBuffer == null)
				// No write buffer assigned to this connection yet; allocate
				// one.
				writeBuffers.put(socketChannel, writeBuffer = ByteBuffer.allocate(WRITE_BUFFER));

			// Perform translation from application data to network data and out
			// the result.
			...
			if (writeBuffer.position() == 0)
				// Wrote all queued data, no longer interested in writing until
				// we receive more application data.
	 ...
	 * 
	 * @param dataBuffer
	 *            The buffer that contains the application data, ready to be
	 *            written (position set to zero, limit set to the end of the
	 *            application data).
	 * @param socketChannel
	 *            The channel over which the network data will be sent.
	 * @param writeBuffer
	 *            The buffer that contains the connection's network data to be
	 *            written, ready to be written/appended to (position set to the
	 *            end of the network data). At the end of this operation, this
	 *            buffer will be ready to be read/sent/written out (position set
	 *            to zero, limit set to the end of the network data).
	 * 
	 * @return The (possibly new) writeBuffer.
	 * 
	 * @throws IOException
	 *             If the data buffer couldn't be wrapped by the SSL engine.
	 */
	private ByteBuffer fromApplicationData(final ByteBuffer dataBuffer, final SocketChannel socketChannel, final ByteBuffer writeBuffer) throws IOException {

		ByteBuffer newWriteBuffer = writeBuffer;

		SSLEngine sslEngine = sslEngines.get(socketChannel);
		if (sslEngine != null) {
			// SSL/TLS: Encrypt network data from application data (or from
			// nothingness!).
			...
			while (true) {
				...
				case BUFFER_UNDERFLOW:
					// Not enough application data collected for a whole SSL/TLS
					// packet.
				...
				case OK:
					logger.dbg("[>>>>: %s] SSL %s - %s: Consumed %d bytes application data", //
							nameChannel(socketChannel), sslEngineResult.getStatus(), sslEngineResult.getHandshakeStatus(), sslEngineResult.bytesConsumed());
					break;
				}
		...
		// Plain Text: Copy application data to network data and prepare both
		// buffers for their next operations.
		if (newWriteBuffer.remaining() < dataBuffer.remaining()) {
			// Not enough space in the writeBuffer for the dataBuffer's data;
			// make it bigger.
	 ...
	 * @param resetByPeer
	 *            <code>true</code> if the channel was closed by the remote
	 *            party (inbound will be closed). <code>false</code> if the
	 *            channel is closed by us (outbound will be closed).
	 * 
	 * @throws IOException
	 *             When the channel couldn't be closed cleanly.
	 */
	private void closeChannel(final SocketChannel socketChannel, final boolean resetByPeer) throws IOException {
	...
	/**
	 * Queue a message to be sent to the given destination. The message will be
	 * added to the destination's write queue.
	 * <p/>
	 * <p>
	 * After this process, the application's data buffer's position will be set
	 * right after the bytes that have been queued on the network. This is
	 * guaranteed to be the buffer's limit (eg. all data will be queued).
	 * </p>
	 * <p/>
	 * The connection's queue buffer's position will be right after the newly
	 * added bytes.
	 * 
	 * @param dataBuffer
	 *            A byte buffer that holds the bytes to dispatch. Make sure that
	 *            the buffer is set up read for reading (put the position at the
	 *            start of the data to read and the limit at the end). The
	 *            buffer will be flipped so its position should be right after
	 *            the bytes to write. Use <code>null</code> to request a
	 *            connection shutdown.
	 * @param socketChannel
	 ...
	 */
	public void queue(final ByteBuffer dataBuffer, final SocketChannel socketChannel) throws ClosedChannelException {

		checkArgument(socketChannel.keyFor(selector) != null, "Tried to queue a message for a destination (%s) that is not managed by our selector.", nameChannel(socketChannel));
		...
		synchronized (writeQueueBuffers) {
			...
			if (writeQueueBuffer == null)
				...
				writeQueueBuffers.put(socketChannel, writeQueueBuffer = ByteBuffer.allocate(Math.max(dataBuffer.remaining(), WRITE_QUEUE_BUFFER)));
			else if (writeQueueBuffer.remaining() < dataBuffer.remaining())
				// Not enough space left in the writeQueueBuffer for the
				// application data, make it bigger.
				synchronized (writeQueueLocks.get(socketChannel)) {
					ByteBuffer newWriteQueueBuffer = ByteBuffer.allocate(Math.max(writeQueueBuffer.position() + dataBuffer.remaining(), writeQueueBuffer.capacity() + WRITE_QUEUE_BUFFER));
					writeQueueBuffer.flip();
	 ...
	 * <p/>
	 * <p>
	 * <b>NOTE: You should always use this method and never close the socket or
	 * channel directly!</b><br>
	 * This method makes sure that closure is handled gracefully according to
	 * the wishes of the transport and optional encryption protocols.
	 * </p>
	 * <p/>
	 * <p>
	 * Non-{@link SocketChannel}s (such as the {@link ServerSocketChannel}) may
	 * be closed directly.
	 * </p>
	 * 
	 * @param socketChannel
	 ...
	 */
	public void close(final SocketChannel socketChannel) throws IOException {
	 ...
	 * 
	 * @throws ClosedChannelException
	 *             If applying ops on a closed channel (that is not yet
	 *             registered with the network selector).
	 */
	private void addOps(final SelectableChannel channel, final int... addOps) throws ClosedChannelException {
	 ...
	 *             registered with the network selector).
	 */
	private void delOps(final SelectableChannel channel, final int... delOps) throws ClosedChannelException {
	...
	/**
	 * Register the given operations with a channel. It is guaranteed that only
	 * operations that are valid for the type of channel will be applied to it.
	 * The operations will be queued and applied to the channel as soon as it is
	 * available.
	 * 
	 * @param channel
	 ...
	 *             registered with the network selector).
	 */
	private synchronized void setOps(final SelectableChannel channel, final int... newOps) throws ClosedChannelException {
	...
	/**
	 * Determine the interested operations for SSL enabled channels. The SSL
	 * protocol can request read or write operations depending on what it needs
	 * to complete/initiate a handshake.
	 * 
	 * @throws ClosedChannelException
	 *             If applying ops on a closed channel (that is not yet
	 *             registered with the network selector).
	 */
	private void processHandshakes() throws ClosedChannelException {
	 ...
	 * 
	 * @throws IOException
	 *             If any I/O errors occur during reading from and writing to
	 *             channel sockets or performing SSL wrapping.
	 */
	 ...
	 * 
	 * @throws IOException
	 *             When a channel couldn't be closed cleanly. The operation was
	 *             aborted and subsequent channels haven't been closed.
	 */
	private void processClosure() throws IOException {
	 ...
	 * 
	 * @param dataBuffer
	 *            The buffer that contains the data which was received. The
	 *            buffer has been flipped and is ready to be read from. You can
	 *            flip it again after you read from it if you want to read the
	 *            same data again.
	 * @param socketChannel
	 ...
	 * @param resetByPeer
	 *            <code>true</code> if the remote side closed the connection,
	 *            <code>false</code> if the local side hung up.
	 */
	...
	/**
	 * Register a {@link NetworkDataListener} to be notified when new data
	...
	/**
	 * Unregister a {@link NetworkDataListener} so that it is no longer being
	 * notified data events on the network.
	 * 
	 * @param listener
	...
	/**
	 * Register a {@link NetworkServerStateListener} to be notified when server
	 * channel state changes occur on the network.
	 * 
	 * @param listener
	 *            The object that wishes to be notified.
	 */
	...
	/**
	 * Unregister a {@link NetworkServerStateListener} so it is no longer
	 * notified when server channel state changes occur on the network.
	 * 
	 * @param listener
	 *            The object that no longer wishes to be notified.
	 */
	...
	/**
	 * Register a {@link NetworkConnectionStateListener} to be notified when
	 * connection state changes occur on the network.
	 * 
	 * @param listener
	 *            The object that wishes to be notified.
	 */
	...
	/**
	 * Unregister a {@link NetworkConnectionStateListener} so it is no longer
	 * notified when connection state changes occur on the network.
	 * 
	 * @param listener
	 *            The object that no longer wishes to be notified.
	 */
			...
			try {
				try {
					...
					while (keysIt.hasNext()) {
						...
						if (key.channel() instanceof ServerSocketChannel) {
							...
							if (key.isValid() && key.isAcceptable()) {
								// Listening channel received connection
								// request.
					...
					if (--errorThrottle <= 0)
						...
						// Throttle down retries by one second longer for each
						// new error we receive.
						synchronized (this) {
	 ...
	 * 
	 * @return A (short) string representation of the connection/socket on the
	 *         given channel.
	 */
